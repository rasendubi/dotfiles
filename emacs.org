...
#+end_src
** Color scheme
I use monokai and airline with molokai color scheme.
...
(use-package monokai-theme
  ...
  (custom-theme-set-faces
   ...
   ;; If org-block-end-line happens to be the last line in the collapsed
   ;; tree, the whole line after ellipsis is highlighted in the
   ;; background color, which is not nice.
   ...
   ;; the minibuffer-prompt background is probably overwritten by
   ;; airline theme (so the text is hard to read)
   `(minibuffer-prompt ((t :foreground ,monokai-blue :background nil)))))
#+end_src
** Font
#+begin_src emacs-lisp
