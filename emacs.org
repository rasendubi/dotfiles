...
(general-create-definer --s-leader-def  :prefix "s-SPC")
...
(general-create-definer s-leader-def :prefix "SPC" :global-prefix "s-SPC" :keymaps '(motion))
#+end_src
** Other
...
(use-package exwm-randr
  ...
  (add-hook 'exwm-randr-screen-change-hook
            (lambda ()
              (rasen/start-command "autorandr -c")))

  ;;          (lambda ()
  ;;            (start-process-shell-command
  ;;             "xrandr" nil "xrandr --output DP-1 --mode 3840x2160 --pos 0x0 --rotate normal --output eDP-1 --mode 3200x1800 --pos 320x2160 --rotate normal")))
...
(exwm-input-set-key (kbd "s-1") #'rasen/exwm-firefox)
(exwm-input-set-key (kbd "s-4") #'rasen/exwm-telegram)
(exwm-input-set-key (kbd "s-7") #'rasen/exwm-google-play-music)
(exwm-input-set-key (kbd "s-9") #'notmuch)
(exwm-input-set-key (kbd "<s-return>") #'eshell)
(exwm-input-set-key (kbd "<s-S-return>") #'rasen/terminal)
(exwm-input-set-key (kbd "s-M-l") #'rasen/lock-screen)
(exwm-input-set-key (kbd "<print>") #'rasen/screenshot)
...
#+end_src
** Razer
Handle input from Razer mouse. (See naga package in top-level README.)
#+begin_src emacs-lisp
  ...
  (with-current-buffer (window-buffer (selected-window))
    ...
    (when (string= value "down")
      (cond ((= code 11) (rasen/switch-to-previous-buffer))
            ...
            (t (message "razer-unhandled %s" value))))))
#+end_src
* Evil
** General
#+begin_src emacs-lisp
...
#+end_src
** Disable GC in minibuffer
#+begin_src emacs-lisp
(defun rasen/minibuffer-setup-hook ()
  (setq gc-cons-threshold most-positive-fixnum))
(defun rasen/minibuffer-exit-hook ()
  (setq gc-cons-threshold 800000))

(add-hook 'minibuffer-setup-hook #'rasen/minibuffer-setup-hook)
(add-hook 'minibuffer-exit-hook  #'rasen/minibuffer-exit-hook)
#+end_src
* Org-mode
** General
#+begin_src emacs-lisp
...
(use-package org-roam
  ...
  (setq org-roam-capture-templates
        '(("d" "dynamic (timestamp)" plain (function org-roam--capture-get-point)
           ...
           :head "#+TITLE: ${title}\n"
           :unnarrowed t)
          ("r" "ring (timestamp)" plain (function org-roam--capture-get-point)
           "%?"
           :file-name "ring/%(rasen/tsid)"
           :head "#+TITLE: ${title}\n"
           :unnarrowed t)))
